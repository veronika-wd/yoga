...
@push('styles')
    <style>
        ...
        .nav-tab.active { color: var(--color-text-dark); }
        ...
        .badge-active { background: rgba(90, 143, 90, 0.1); color: var(--color-success); }
        .badge-pending { background: rgba(139, 115, 85, 0.1); color: var(--color-primary); }
        .badge-expired { background: rgba(154, 149, 144, 0.15); color: var(--color-muted); }
        .badge-rejected { background: rgba(192, 57, 43, 0.1); color: var(--color-danger); }
        ...
        .request-item:hover { border-color: var(--color-secondary); }
        ...
        @media (max-width: 768px) {
            ...
            .request-status { text-align: left; min-width: auto; }
        }
    </style>
@endpush
                        ...
                        @foreach($courses as $course)
                            <div class="card">
                                ...
                                <div class="card-footer">
                                    <span class="meta-small" style="font-size:13px;color:var(--color-muted);font-weight:300;">Прогресс: {{ $course->progress ?? 0 }}%</span>
                                    <a href="{{ route('course.show', $course->id) }}" class="btn-outline">Перейти</a>
                                </div>
                            </div>
                        @endforeach
                    </div>
                        ...
                        @foreach($subscriptions as $sub)
                            <div class="card">
                                ...
                                <p class="card-meta">
                                    ...
                                    Осталось: {{ $sub->is_active ? max(0, now()->diffInDays($sub->expires_at)) : 0 }} дней
                                </p>
                                <div class="card-footer">
                                    <span class="price" style="font-size:18px;font-weight:400;color:var(--color-text-dark);">{{ number_format($sub->price, 0, '.', ' ') }} ₽</span>
                                    @if(!$sub->is_active)
